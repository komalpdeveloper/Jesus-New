        ...
        Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [
                kDeepBlack,
                // Subtly blend royal blue with the accent to tint the whole scene
                Color.lerp(kRoyalBlue, accent, 0.45)!,
              ],
            ),
          ),
        ),
        // Slow moving nebula swirl
        _NebulaLayer(accent: accent),
        // Twinkling stars
...
class _NebulaLayer extends StatefulWidget {
  final Color accent;
  const _NebulaLayer({required this.accent});
  @override
...
class _NebulaLayerState extends State<_NebulaLayer> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        final t = _ctl.value;
        // Use only brand colors: accent, deep black, royal blue. Avoid over-saturating gold to prevent greenish casts.
        final accentSoft = widget.accent.withValues(alpha: 0.05 + 0.05 * math.sin(t * math.pi * 2));
        final sweep = [
          accentSoft,
          kDeepBlack,
